...
/** Safely parse the response body as JSON. Returns null on empty/non-JSON bodies. */
...
export class CoreBankService {
  ...
  /** Whether credentials have been stored (i.e. user has logged in at least once) */
  hasCredentials(): boolean {
    return this.savedCredentials !== null;
  }

  /** Re-authenticate using saved credentials (called by monitor on session expiry) */
  async reAuthenticate(): Promise<boolean> {
    if (!this.savedCredentials) return false;
    try {
      const result = await this.autoLogin(
        this.savedCredentials.username,
        this.savedCredentials.password
      );
  ...
  async getCaptcha(): Promise<{ imageBase64: string; deviceId: string }> {
    ...
    const data = (await safeJson(res.body)) as CaptchaResponse | null;
    if (!data) throw new Error("Failed to fetch captcha — empty response from bank");
  ...
  async login(
    ...
    const body = (await safeJson(res.body)) as any;
  ...
  private async authenticatedRequest(
    ...
    // Guard against empty or non-JSON responses (connection resets, rate limiting, etc.)
    let data: any;
    try {
    ...
    if (data.result.responseCode === "GW200") {
      // Session expired — clear session so monitor can re-authenticate
      this.session = null;
