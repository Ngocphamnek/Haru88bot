...
class TelegramBot2Service {
  ...
  private setupHandlers() {
    ...
    this.bot.on('callback_query', async (query: TelegramBot.CallbackQuery) => {
      ...
      try {
        // Hiển thị code ngay trong popup tại nhóm, user tự nhập trong nhóm
        await this.bot!.answerCallbackQuery(query.id, {
          text: `🎁 Code: ${code}\n\nGõ ngay trong nhóm: /code ${code}`,
          show_alert: true,
        });
      } catch (err) {
    ...
    // ── Gift code redemption in group ─────────────────────────────────────────
    this.bot.onText(/^\/code\s+(\S+)$/i, async (msg: TelegramBot.Message, match) => {
      if (msg.chat.id !== this.MAIN_GROUP) return;
      const user = msg.from;
      if (!user || !match) return;
      const giftCode = match[1].trim().toUpperCase();
      const userId = user.id.toString();
      try {
        const result = await storage.redeemGiftCode(userId, giftCode);
        if (!result.success) {
          await this.bot!.sendMessage(this.MAIN_GROUP,
            `❌ ${result.message || 'Mã code không hợp lệ!'}`,
            { reply_to_message_id: msg.message_id }
          );
          return;
        }
        ...
        await this.bot!.sendMessage(this.MAIN_GROUP,
          ...
          `👤 Người chơi: ${maskedId}\n` +
          `💰 Nhận được: <b>${result.amount!.toLocaleString('vi-VN')}đ</b>\n` +
          `💳 Số dư mới: <b>${newBalance.toLocaleString('vi-VN')}đ</b>`,
          { parse_mode: 'HTML', reply_to_message_id: msg.message_id }
        );
      } catch (err) {
        console.error('[Bot2] /code error:', err);
        await this.bot!.sendMessage(this.MAIN_GROUP,
  ...
  private async processResults() {
    ...
    // Batch credit winner balances in parallel (group multiple bets per user first)
    if (winnersData.length > 0) {
      const winByUser = new Map<string, number>();
      for (const w of winnersData) {
        winByUser.set(w.userId, (winByUser.get(w.userId) ?? 0) + w.winAmount);
      }
      await Promise.all(
        Array.from(winByUser.entries()).map(async ([uid, totalWin]) => {
          try {
            const currentUserData = await storage.getBotUser(uid);
            const currentBalance = parseFloat(currentUserData?.balance || "0");
            await storage.updateBotUser(uid, { balance: (currentBalance + totalWin).toFixed(2) });
          } catch (balErr) {
            console.error(`❌ Failed to credit balance for winner ${uid}:`, balErr);
          }
        })
      );
    }
